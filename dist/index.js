...
var newsletterSignups = mysqlTable("newsletterSignups", {
  id: int("id").autoincrement().primaryKey(),
...
var contactSubmissions = mysqlTable("contactSubmissions", {
  id: int("id").autoincrement().primaryKey(),
...
async function getUpcomingEvents() {
  ...
  const result = await db.select().from(events).where(gte(events.date, now)).orderBy(events.date);
  return result;
}
async function createNewsletterSignup(data) {
  const db = await getDb();
  ...
  try {
    ...
    if (error.code === "ER_DUP_ENTRY") {
      await db.update(newsletterSignups).set({ subscribed: 1, updatedAt: /* @__PURE__ */ new Date() }).where(eq(newsletterSignups.email, data.email));
      return { success: true };
    }
    throw error;
  }
...
var __dirname = typeof import.meta.dirname !== "undefined" ? import.meta.dirname : path.dirname(fileURLToPath(import.meta.url));
